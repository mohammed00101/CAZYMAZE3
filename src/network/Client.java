...
public class Client extends Connection {
	...
	public void request() {
		new Thread() {
			public void run() {
				try {
					send(null, InetAddress.getByName("255.255.255.255"), portServer);
				...
				while (true) { 
					 if(crazyMaze!=null )
					 {
					  send(crazyMaze.getPlayer1().getdata(),ipServer,portServer);
					 }
						try {
							Thread.sleep(20);
			...
			public void run() {
				...
				while (true) {
					...
					if(ob instanceof InetAddress)
					{
						...
						if (run_first) {
							crazyMaze = new CrazyMaze(500, 500, "crazyMaze", mazeRoute);
							run_first = false;
						} else {
					...
					else if(ob instanceof int [] && crazyMaze !=null)
					{
						...
						crazyMaze.getPlayer2().put(data[3],new Player2(crazyMaze, -1));
